...
export default class BaseRepository {
        ...
        if (relation.split(".").length > 1) {
          ...
          model = model.preload(firstRelation, (query) => {
            if (this.RELATION_OPTIONS) {
              let relationOption = this.RELATION_OPTIONS.find((item: any) => {
                return item.relation == firstRelation;
              });
              this.parseRelationOption(query, relationOption);
            }
            this.parseNestedRelation(
              query,
              relation.substr(relation.indexOf(".") + 1),
              firstRelation
            );
          });
        } else {
  ...
  parseNestedRelation(query: any, relation: string, firstRelation: string) {
    let relations = this.RELATIONS.filter((d) => {
      return typeof d == "string";
    });
    relations = relations.filter((d) => {
      return d.includes(firstRelation + ".");
    });
    if (relations.length > 1) {
      relations.map((data) => {
        this.parseNestedRelation(
          ...
          relation.substr(0, data.indexOf("."))
        );
      });
    } else {
      if (relation.indexOf(".") > 0) {
        ...
        query.preload(subRelation, (subQuery) => {
          if (this.RELATION_OPTIONS) {
            let relationOption = this.RELATION_OPTIONS.find((item: any) => {
              return item.relation == subRelation;
            });
  ...
  parseRelationOption(query: any, relationOption: any) {
    if (relationOption) {
      ...
      if (relationOption.search) {
        query = this.parseSearch(
          query,
          relationOption.filter,
          relationOption.search
        );
      }
    }
    return query;
